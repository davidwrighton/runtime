...
void CallStubGenerator::ComputeCallStub(MetaSig &sig, PCODE *pRoutines, MethodDesc *pMD)
{
    ...
    if (pMD != NULL && pMD->HasUnmanagedCallersOnlyAttribute())
    {
    ...
    if (rewriteMetaSigFromExplicitThisToHasThis)
    {
        ...
        if ((sig.NumFixedArgs() == 0) || sig.HasThis())
        {
        ...
        while(sig.NextArg() != ELEMENT_TYPE_END)
        {
            ...
            else
            {
                ...
                if (!thArgTypeHandle.IsValueType())
                {
                    sigBuilder.AppendElementType(ELEMENT_TYPE_OBJECT);
                }
                else
                {
                    _ASSERTE(thArgTypeHandle.IsValueType());
                    MethodTable *pMTArg = thArgTypeHandle.AsMethodTable();
                    CorElementType elemType = pMTArg->GetInternalCorElementType();
                    if (elemType == ELEMENT_TYPE_VALUETYPE)
                    {
                        sigBuilder.AppendElementType(ELEMENT_TYPE_INTERNAL);
                        sigBuilder.AppendPointer(thArgTypeHandle.AsPtr());
                    }
                    else
                    {
                        sigBuilder.AppendElementType(elemType);
                    }
                }
            }
        }
